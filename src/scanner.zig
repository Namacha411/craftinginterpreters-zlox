...
pub const Scanner = struct {
    ...
    fn checkKeyword(self: *Scanner, keyword: []const u8, t: TokenType) TokenType {
        const eql = @import("std").mem.eql;
        const length = keyword.len;
        if ((self.current - self.start == length) and eql(u8, keyword, self.source[self.start..(self.start + length)])) {
            return t;
        }
    ...
    fn identifierType(self: *Scanner) TokenType {
        switch (self.source[self.start]) {
            ...
            'v' => self.checkKeyword("var", TokenType.var_),
            'w' => self.checkKeyword("while", TokenType.while_),
        }
        return TokenType.identifire;
    }
